...
<div class="coupon_woo_rehub <?php echo $reveal_enabled; echo $coupon_style?>">
  	<?php if ($coupon_mask_enabled =='1') :?>
	  	...
	  	<div class="rehub_offer_coupon masked_coupon coupon_btn re_track_btn <?php if(!empty($offer_coupon_date)) {echo $coupon_style ;} ?>" data-clipboard-text="<?php echo $offer_coupon ?>" data-codeid="<?php echo $product->id ?>" data-dest="<?php echo $offer_url ?>"><?php if(rehub_option('rehub_mask_text') !='') :?><?php echo rehub_option('rehub_mask_text') ; ?><?php else :?><?php _e('Reveal coupon', 'rehub_framework') ?><?php endif ;?>
	  	</div>
	<?php else :?>
