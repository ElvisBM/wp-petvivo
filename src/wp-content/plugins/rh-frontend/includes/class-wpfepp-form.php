...
class WPFEPP_Form
{
		...
		foreach ($this->get_fields() as $key => $field) {
			switch ($field['type']) {
				...
				case 'non_hierarchical_taxonomy':
					...
					break;
				case 'sku':
					if(!empty($post_data[$key])) $custom_fields['_sku'] = $this->sanitize($post_data[$key], $field);
					break;
				case 'price':
					if(!empty($post_data[$key])) $custom_fields['_regular_price'] = $this->sanitize($post_data[$key], $field);
					break;
				case 'sale_price':
					if(!empty($post_data[$key])) {
	...
	public function enqueue_frontend_location_scripts($post_id) {
		...
		wp_localize_script( 'wpfepp-script', 'wpfeppl', array(
			...
			'start_geo_long' => ($current_start_geo_long && !empty($current_start_geo_long) ) ? $current_start_geo_long : $this->extended['start_geo_long'],
